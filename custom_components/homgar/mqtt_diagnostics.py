...
class HomGarMQTTDiagnosticsSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, sensor_key: str, device_info: dict, field_name: str):
        ...
        mid = device_info.get("mid")
        self._attr_unique_id = f"rainpoint_hub_{mid}_mqtt_{field_name}"
        self._attr_name = f"{device_info.get('sub_name', 'MQTT')} {field_name.replace('_', ' ').title()}"
        self._attr_entity_registry_enabled_default = False

    @property
    def device_info(self) -> DeviceInfo:
        """Return device information — attaches to the hub device."""
        return {
            "identifiers": {(DOMAIN, self._sensor_key)},  # rainpoint_hub_{mid}
...
async def async_setup_mqtt_diagnostics(
    hass: HomeAssistant,
    entry: ConfigEntry,
) -> None:
    """Set up MQTT diagnostic sensors, called directly from async_setup_entry."""
    ...
    if entities:
        component: EntityComponent = hass.data.get(SENSOR_DOMAIN)
        if component:
            await component.async_add_entities(entities)
            _LOGGER.info("Added %d MQTT diagnostic sensors", len(entities))
        else:
            _LOGGER.warning("HomGar MQTT diagnostics: sensor component not ready")
    else:
